...
function rebuildRecordingSessionsTable(db: Database.Database): void {
  ...
  db.prepare(`
    INSERT OR REPLACE INTO recording_sessions_new (
      id, name, mode, status, source_tool, environment_id, account_id, role, target_fields,
...
function rebuildTestRunDraftsTable(db: Database.Database): void {
  ...
  db.prepare(`
    CREATE TABLE IF NOT EXISTS test_run_drafts_new (
      ...
      name TEXT NOT NULL,
      status TEXT DEFAULT 'preconfigured' CHECK (status IN ('generated', 'preconfigured', 'published')),
      sequence INTEGER,
  ...
  if (hasPublishedPresetId) {
    db.prepare(`
      INSERT OR REPLACE INTO test_run_drafts_new (
        id, session_id, name, status, sequence, source_event_id, summary, draft_payload,
        published_preset_id, published_test_run_id, created_at, updated_at
      )
      SELECT
        id, session_id, name,
        CASE
          WHEN status = 'generated' THEN 'preconfigured'
          ELSE status
        END,
...
export class SqliteProvider implements DbProvider {
  ...
  private initRepositories(): void {
    ...
    this.repos = {
      ...
      recordingSessions: createSqliteRepository(this.db, 'recording_sessions', ['target_fields', 'summary'], []),
