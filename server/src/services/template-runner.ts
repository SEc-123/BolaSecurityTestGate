    ...
    for (const template of templates) {
      ...
      for (const combination of combinations.length > 0 ? combinations : [{ values: {}, accountMap: {}, victimIds: [], varConfigs: {} }]) {
        ...
        if (enableBaseline && bindingStrategy === 'anchor_attacker' && attacker && combination.attackerId) {
          ...
          const baselineRequest = buildRequest(parsedRequest, baselineCombination, variables);
          const baselineUrl = baseUrl + baselineRequest.path;
          ...
          if (shouldCreateFinding) {
            ...
            if (dropResult.dropped) {
              ...
              const finding: Omit<Finding, 'id' | 'created_at' | 'updated_at'> = {
                ...
                baseline_response: baselineResponse ? truncateResponseForStorage(baselineResponse) : undefined,
                mutated_response: truncateResponseForStorage(mutatedResponse),
