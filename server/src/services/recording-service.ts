...
function rebuildTestRunDraftArtifacts(
  ...
  return {
    summary: {
      ...
      method,
      path,
      environment_id: presetPayload.environment_id || undefined,
      account_id: presetPayload.default_account_id || undefined,
    ...
    draft_payload: {
      ...
      preset: {
        ...
        description: presetPayload.description || 'Optional reusable run preset generated from this draft',
        environment_id: presetPayload.environment_id || '',
        default_account_id: presetPayload.default_account_id || '',
        preset_config: {
...
async function refreshTestRunDraftMaterialization(db: DbProvider, draftId: string): Promise<TestRunDraft> {
  ...
  const updated = await db.repos.testRunDrafts.update(draft.id, {
    ...(rebuilt as any),
    draft_status: draft.draft_status || draft.status || 'generated',
    intent: draft.intent || 'api_test_seed',
  } as any);
  if (!updated) {
...
export async function createRecordingSession(db: DbProvider, data: {
  name: string;
  mode?: 'workflow' | 'api';
  intent?: 'account_capture' | 'api_test_seed' | 'workflow_seed' | 'learning_seed';
  source_tool?: string;
  ...
  const derivedMode = data.mode || (data.intent === 'account_capture' || data.intent === 'api_test_seed' ? 'api' : 'workflow');
  const derivedIntent = data.intent || (derivedMode === 'api' ? 'api_test_seed' : 'workflow_seed');

  if (!['workflow', 'api'].includes(derivedMode)) {
  ...
  const session = await db.repos.recordingSessions.create({
    ...
    requested_field_names: (data.requested_field_names || []).map(value => String(value || '').trim()).filter(Boolean),
    capture_filters: data.capture_filters || {},
...
export async function promoteTestRunDraftToTestRun(db: DbProvider, draftId: string, params?: {
  ...
  const defaultAccountId = typeof presetPayload.default_account_id === 'string'
    ? presetPayload.default_account_id.trim()
    : '';
  const accountIds = defaultAccountId ? [defaultAccountId] : [];
  const environmentId = typeof presetPayload.environment_id === 'string'
    ? presetPayload.environment_id.trim()
    : undefined;
...
export async function publishTestRunDraft(db: DbProvider, draftId: string, params?: {
  ...
  const preset = await db.repos.testRunPresets.create({
    ...
    source_draft_id: draft.id,
    template_id: template.id,
    environment_id: typeof presetPayload.environment_id === 'string' && presetPayload.environment_id.trim()
      ? presetPayload.environment_id.trim()
      : undefined,
    default_account_id: typeof presetPayload.default_account_id === 'string' && presetPayload.default_account_id.trim()
      ? presetPayload.default_account_id.trim()
      : undefined,
    preset_config: {
      ...(presetPayload.preset_config || {}),
