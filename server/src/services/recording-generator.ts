    ...
    hits
      ...
      .map(hit => {
        ...
        return {
          ...
          confidence: hit.confidence || 0.78,
          reason: hit.bind_to_account_field ? 'Matched explicit account-bound field from recorded request' : 'Matched request field suitable for parameterization',
...
export function generateApiDraftArtifacts(params: {
  ...
  function buildApiAssertionCandidates(event: RecordingEvent): JsonRecord[] {
    ...
    if (event.response_status) {
      candidates.push({
        ...
        confidence: 0.92,
        reason: 'Derived from the successful recorded response status',
    ...
    if (contentType) {
      candidates.push({
        ...
        value: contentType,
        confidence: 0.7,
        reason: 'Derived from a stable response content-type header',
      });
    }
    ...
    if (responseBody && typeof responseBody === 'object' && !Array.isArray(responseBody)) {
      for (const [key, value] of Object.entries(responseBody)) {
        ...
        candidates.push({
          ...
          value: String(value),
          confidence: 0.62,
          reason: 'Derived from a scalar business field found in the successful response body',
        });
      }
    }
  ...
  const drafts = [...events]
    ...
    .map(event => {
      ...
      const failurePatternSuggestions = event.response_status ? [{
        id: `failure:${event.id}:status`,
        type: 'http_status',
        operator: 'not_equals',
        value: String(event.response_status),
        confidence: 0.9,
        reason: 'Negating the successful response status is the default first failure signal',
      }] : [];
      const confidenceValues = [
        ...requestVariables.map(item => Number(item.confidence || 0)),
        ...assertionCandidates.map(item => Number(item.confidence || 0)),
        ...failurePatternSuggestions.map(item => Number(item.confidence || 0)),
      ].filter(value => Number.isFinite(value));
      const draftConfidence = confidenceValues.length > 0
      ...
      return {
        ...
        intent: 'api_test_seed' as const,
        draft_status: 'generated' as const,
        sequence: event.sequence,
        source_event_id: event.id,
        summary: {
          method: event.method,
          ...
          failure_pattern_count: failurePatternSuggestions.length,
          draft_confidence: draftConfidence,
        },
        suggestion_summary: {
          confidence: draftConfidence,
          variable_suggestion_count: requestVariables.length,
          assertion_suggestion_count: assertionCandidates.length,
          failure_pattern_suggestion_count: failurePatternSuggestions.length,
          response_fingerprint_summary: responseSnapshot,
          source_event_id: event.id,
        },
        review_decisions: {},
        draft_payload: {
          ...
          template: {
            ...
            failure_patterns: failurePatternSuggestions,
            failure_logic: 'OR',
            account_binding_suggestions: [],
            is_active: true,
          },
          preset: {
