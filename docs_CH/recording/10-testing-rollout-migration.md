...
```bash
# Cross-platform Node entry (supports both POSIX and legacy PowerShell-style flags)
...
```text
scripts/recording-db-migrate.mjs
兼容旧版 PowerShell：`scripts/recording-db-migrate.ps1`
```
