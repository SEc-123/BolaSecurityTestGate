...
public final class BstgApiClient {
    ...
    public CreateSessionResult createSession(SettingsStore.RecorderSettings settings, CreateSessionInput input) throws IOException, InterruptedException {
        ...
        payload.put("intent", input.intent());
        ...
        if (!input.accountLabel().isBlank()) {
            payload.put("account_label", input.accountLabel());
        }
        if (!input.requestedFieldNames().isBlank()) {
            payload.put("requested_field_names", java.util.Arrays.stream(input.requestedFieldNames().split(",")).map(String::trim).filter(s -> !s.isBlank()).toList());
        }
        if (!input.captureFilters().isBlank()) {
            payload.put("capture_filters", Map.of("raw", input.captureFilters()));
        }
        ...
        return new CreateSessionResult(
            ...
            getString(data, "intent", input.intent())
        );
    }
    ...
    public record CreateSessionResult(String sessionId, String sessionName, String mode, String intent) {
    }
