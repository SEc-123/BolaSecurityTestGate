...
export function TestRunDraftEditorModal({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Edit API Draft: ${draft.name}`}
      size="xl"
      footer={(
        <>
          ...
          <Button
            onClick={() => onSave({
              name: optionalValue(draftName),
              template: {
                name: optionalValue(templateName),
                description: optionalValue(templateDescription),
                variables,
                failure_patterns: failurePatterns,
                failure_logic: failureLogic,
              },
              preset: {
                name: optionalValue(presetName),
                description: optionalValue(presetDescription),
                environment_id: optionalValue(environmentId),
                default_account_id: optionalValue(defaultAccountId),
              },
            })}
            loading={saving}
          >
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Input
            ...
            placeholder="Reusable preset name"
          />
          <Select
        ...
        <TextArea
          ...
          rows={2}
          placeholder="Describe when to use this preconfigured run"
        />
