...
const RECORDING_INTENT_OPTIONS: Array<{ value: RecordingIntent; label: string; mode: RecordingMode }> = [
  { value: 'account_capture', label: 'Account Capture', mode: 'api' },
  { value: 'api_test_seed', label: 'API Test Seed', mode: 'api' },
  { value: 'workflow_seed', label: 'Workflow Seed', mode: 'workflow' },
  { value: 'learning_seed', label: 'Learning Seed', mode: 'workflow' },
];
...
export function Recordings({ onOpenDetail, rolloutConfig }: RecordingsProps) {
  ...
  const [showOptionalSessionContext, setShowOptionalSessionContext] = useState(false);
  ...
  function resetCreateForm() {
    ...
    setFormData({
      name: '',
      mode: defaultMode,
      intent: defaultMode === 'api' ? 'api_test_seed' : 'workflow_seed',
      account_label: '',
      requested_field_names: '',
      environment_id: '',
      account_id: '',
    ...
    setShowOptionalSessionContext(false);
  ...
  async function handleCreateSession() {
    ...
    try {
      const created = await recordingsService.createSession({
        ...
        requested_field_names: formData.requested_field_names.split(',').map(value => value.trim()).filter(Boolean),
        capture_filters: formData.intent === 'account_capture'
          ? { prefer_json_api_responses: true, ignore_static_assets: true, only_capture_selected_host: true }
          : undefined,
  ...
  return (
    <div className="space-y-6 p-8">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-medium text-gray-700">
                      {(session.intent || (session.mode === 'api' ? 'api_test_seed' : 'workflow_seed')).replace(/_/g, ' ')}
                    </span>
                  </div>
                </div>
                <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-gray-500">
                  {session.environment_id && <span>Environment Hint: {environmentMap.get(session.environment_id || '') || session.environment_id}</span>}
                  {session.account_id && <span>Account Hint: {accountMap.get(session.account_id || '') || session.account_id}</span>}
      ...
      <Modal
        ...
        title="Start Recording For Template Capture"
        size="xl"
        footer={(
          <>
        ...
        <div className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            <Input
              ...
              placeholder="e.g., Buyer account capture"
            />
            <Select
              label="Recording Intent"
              value={formData.intent}
              onChange={event => {
                const nextIntent = event.target.value as RecordingIntent;
                const matching = RECORDING_INTENT_OPTIONS.find(item => item.value === nextIntent);
                setFormData(prev => ({ ...prev, intent: nextIntent, mode: matching?.mode || prev.mode }));
              }}
              options={RECORDING_INTENT_OPTIONS.filter(item => (item.mode === 'api' ? rolloutConfig.api_mode_enabled : rolloutConfig.workflow_mode_enabled)).map(item => ({ value: item.value, label: item.label }))}
            />
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Input
              ...
              onChange={event => setFormData(prev => ({ ...prev, account_label: event.target.value }))}
              placeholder="buyer-us-east"
            />
            <Input
              label="Requested Field Names (optional)"
              value={formData.requested_field_names}
              onChange={event => setFormData(prev => ({ ...prev, requested_field_names: event.target.value }))}
              placeholder="userId, username, email, tenantId"
            />
          </div>

          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-900">
            Start with just a session name and mode. Environment, account, and field binding are optional hints for later runs and account linkage, not required to save templates.
          </div>

          <section className="rounded-xl border border-gray-200 bg-gray-50 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-base font-semibold text-gray-900">Optional Runtime Context</h3>
                <p className="text-sm text-gray-500">Only fill this if you want the recording session to remember a default environment, account, or role for later execution. It is not required for template saving.</p>
              </div>
              <Button variant="secondary" size="sm" onClick={() => setShowOptionalSessionContext((current) => !current)}>
                {showOptionalSessionContext ? 'Hide' : 'Configure'}
              </Button>
            </div>

            {showOptionalSessionContext && (
          ...
          <section className="rounded-xl border border-gray-200 bg-gray-50 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-base font-semibold text-gray-900">Optional Target Fields</h3>
                <p className="text-sm text-gray-500">Use this only when you want recording to pre-link tokens, user IDs, or account fields automatically.</p>
              </div>
              <div className="flex gap-2">
                ...
                <Button variant="secondary" size="sm" onClick={() => setShowTargetFields((current) => !current)}>
                  {showTargetFields ? 'Hide' : 'Configure'}
                </Button>
              </div>
            </div>
            ...
            {showTargetFields && (
              <div className="mt-4 space-y-3">
                {targetFields.map((target, index) => (
                  <div key={`target-${index}`} className="rounded-xl border border-gray-200 bg-white p-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      <Input
                        label="Field Name"
                        value={target.name}
