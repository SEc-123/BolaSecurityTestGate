...
export function RecordingDetail({
  ...
  const [wizardEventId, setWizardEventId] = useState<string | undefined>(undefined);
  ...
  const [showRuntimeHints, setShowRuntimeHints] = useState(false);
  const [isAccountDraftModalOpen, setIsAccountDraftModalOpen] = useState(false);
  ...
  function handleOpenWorkflowLearning(workflowId: string) {
    try {
      window.localStorage.setItem('bstg.workflowLearning.prefill', JSON.stringify({
        workflowId,
        source: 'recording_only',
  ...
  async function handleRegenerateAccountDraft() {
    try {
      setBusyAction('regenerate-account-draft');
  ...
  async function handlePublishAccountDraft(payload: RecordingAccountDraftSubmitPayload) {
    try {
      setBusyAction('publish-account-draft');
      const result = await recordingsService.publishAccount(sessionId, {
        ...payload,
        published_by: 'recording_detail',
      });
      if (result.account) {
        setAccounts(current => {
          const existingIndex = current.findIndex(account => account.id === result.account?.id);
          if (existingIndex === -1 && result.account) return [result.account, ...current];
  ...
  const nextStepText = session.mode === 'workflow'
    ...
    : 'Review the generated API template drafts below. Save the template first, then optionally create presets or runnable test runs later.';
    ...
    <div className="space-y-6 p-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600">
            Recording detail is the confirmation desk for captured events and generated drafts. Runtime hints and account linkage are optional helpers, not part of the template itself.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button variant="secondary" onClick={() => setShowRuntimeHints((current) => !current)}>
            <Link2 size={16} className="mr-2" />
            {showRuntimeHints ? 'Hide Runtime Hints' : 'Show Runtime Hints'}
          </Button>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button variant="secondary" size="sm" onClick={handleOpenApplyModal}>
      ...
      <RecordingEventTimeline
        ...
        onGenerateApiTestDraft={session.mode === 'api' ? ((eventId) => {
          setWizardEventId(eventId);
          setIsApiDraftWizardOpen(true);
        }) : undefined}
      />
            ...
            <div className="space-y-4">
              {detail.workflow_drafts.map(draft => (
                <WorkflowDraftPreview
                  ...
                  onOpenWorkflowLearning={handleOpenWorkflowLearning}
                />
              ))}
            </div>
          )
      ...
      <Modal
        isOpen={isAccountDraftModalOpen}
        onClose={() => setIsAccountDraftModalOpen(false)}
        title="Publish Recording As Account"
        size="xl"
        footer={<Button variant="secondary" onClick={() => setIsAccountDraftModalOpen(false)}>Close</Button>}
      >
        <RecordingAccountDraftReview
          draft={detail.account_draft || null}
          accounts={accounts}
          loading={busyAction === 'regenerate-account-draft'}
          saving={busyAction === 'publish-account-draft'}
          onRegenerate={() => { void handleRegenerateAccountDraft(); }}
      ...
      <ApiTestDraftWizard
        ...
        onOpenWorkspace={(draftId) => onOpenPreconfiguredRuns?.({ draftId })}
        onRunCreated={(runId) => onOpenTestRuns?.(runId)}
      />
